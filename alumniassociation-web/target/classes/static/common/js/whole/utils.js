...
(function ($, window) {
     ...
     * @Date: 2017/12/8
     */
    window.$t.closeRefresh=function () {
        var iframes = $(parent.document).find("iframe");
        var parent_iframe = null;
        var array = [];
        if (iframes) {
            $.each(iframes, function (i, val) {
                var regex = new RegExp('layui-layer-iframe');
                if (regex.test(val['id'])) {
                    //parent_iframe = iframes[i];
                    array.push(iframes[i]);
                }
            });
        }
        if(array.length>1){
            parent_iframe = array[array.length-2];
        }else{
            parent_iframe = array[array.length-1];
        }

        var  frameindex= parent.layer.getFrameIndex(window.name);
        parent.layer.close(frameindex);
        var parentIfame = $(parent_iframe).contents();
        parentIfame.find(".search-btn").click();
        if (iframes) {
            $.each(iframes, function (i, val) {
                var regex = new RegExp('cy-show');
                if (regex.test(val['className'])) {
                    parent_iframe = iframes[i];
                    return false;
                }
            });
     ...
     * @Date: 2017/12/8
     */
    window.$t.closeRefreshNotUpdate=function () {
        var iframes = $(parent.document).find("iframe");
        var parent_iframe = null;
        var array = [];
        if (iframes) {
