...
public class SysUserController extends AbstractController {
	...
	public R save(@RequestBody SysUser user){
		...
		//String orgId = sysRoleService.queryOrgIdByRoleID(user.getRoleIdList().get(0));
		user.setPassword(DefaultEnum.PASSWORD.getCode());
		user.setCreateUserId(getUserId());
		user.setHeadImage("http://web.fsyizhuang.com/upload/image/headImg/default.jpg");
		//user.setOrgId(orgId);
     ...
     */
    @ResponseBody
    @RequestMapping("/worksUser")
    @RequiresPermissions("sys:user:worksUser")
    public R worksUser(@RequestParam Map<String, Object> params){
        //只有超级管理员，才能查看所有管理员列表
        if(getUserId() != Constant.SUPER_ADMIN){
        ...
        //查询列表数据
        Query query = new Query(params);
        List<SysUser> userList = sysUserService.queryWorksList(query);
        int total = sysUserService.queryWorksCount(query);
