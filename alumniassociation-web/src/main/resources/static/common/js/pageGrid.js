        ...
        renderData: function (R, $grid, pageProps) {
            ...
            for (var i = 0; i < data.length; i++) {
                    ...
                    if (operate == "true") {
                        ...
                        for (var key in data[i]) {
                            if (key == _name) {
                                ...
                                //如果有渲染的方法 先进去渲染方法
                                if (click != null && click != undefined && click != "") {
                                    var funcname = click.split(";");
                                    for(var action of funcname){
                                        var fname = action.substring(0,action.indexOf("("));
                                        var cal = action.substring(action.indexOf("(")+1,action.indexOf(")"));
                                        if(cal.indexOf(".")!=-1){
                                            var key1 = cal.split(".")[1];
                                            click = "onclick='"+fname+'('+ rowdata[key1] +')'+"'";
                                        }else{
                                            click = "onclick='"+fname+'('+ rowdata +')'+"'";
                                        }
                                    }
                                }
                                ...
                                if (hide == "true") {
                                    ...
                                    if(click!=''){
                                        $(_tr).append('<td name=' + _name +'  '+ click +'>' + value + '</td>');
                                    }else{
                ...
                if (isCheckbox == "true") {
                    ...
                    var checkboxTd = null;
                    if(rowdata.checked){
                    	checkboxTd = '<td><input type="checkbox"   primary="' + primary + '" lay-skin="primary" checked></td>';
                    }else{
                    	checkboxTd = '<td><input type="checkbox"   primary="' + primary + '" lay-skin="primary"></td>';
                    }
                    //复选框监听标识
                    if (filter != "") {
