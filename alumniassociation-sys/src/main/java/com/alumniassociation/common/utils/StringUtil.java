...
public class StringUtil {
    ...
    /**
	 * @author lewp
	 * @Description String[] 转成串，用逗号隔开
	 * @param
	 * @date 2018-12-28 上午14:09:00
	 */
    public static String arrayToStr(String[] ss){  
    	if(ss == null){
    		return "";
    	}
	...
	public static String getSequenceName(String fileName) {
		...
		if(fileName == null || fileName.lastIndexOf(".") < 0){
			...
			return getRandomString(4) + System.currentTimeMillis() + fileName.substring(fileName.lastIndexOf("."));
		}
	 ...
	 * @return
	 */
	public static String getNumSequenceName(String fileName) {
		try {
			Thread.sleep(1);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if("".equals(fileName) || fileName == null || fileName.lastIndexOf(".") < 0){
	...
	public static String inputStream2String(InputStream in, String encoding) {
		...
		try {
			while ((size = in.read(buffer)) != -1) {
				sb.append(new String(buffer, 0, size, encoding));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
