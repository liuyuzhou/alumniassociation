...
public class DateUtil {
	...
	private static SimpleDateFormat SDF_CHINA_MD = new SimpleDateFormat("MM月dd日");
	...
	public static String getYmdhm(Date date) {
		return SDF_YMDHM.format(date == null ? new Date() : date);
	}
	...
	public static Date defaultDateStrToDate(String dateStr){
    ...
    public static Date changeYMDHMSToDate(String dateStr) {
        try {
    ...
    public static Date changeYMDHMToDate(String dateStr) {
        try {
            return  SDF_YMDHM.parse(dateStr);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return null;
...
//			Date dt2 = SDF_Y_M_D.parse(DATE2);
	 ...
	 * @author zenghaohui
	 * @date 11/3/2019 上午 11:52
	 */
	public static String getTodayString(){
		String today = Instant.now().atZone(ZoneId.systemDefault()).format(DATE_TIME_FORMATTER);
		return today;
	}
	...
	public static String getTomorrowString(){
		String tomorrow = Instant.now().atZone(ZoneId.systemDefault()).plusDays(1).format(DATE_TIME_FORMATTER);
		return tomorrow;
	}
